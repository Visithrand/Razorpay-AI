          ...
          <div className="bg-white border rounded-xl shadow-xl overflow-hidden flex flex-col max-h-[65vh]" style={{ borderColor: 'var(--rz-border)' }}>
            ...
            <div className="px-5 py-4 border-b flex-shrink-0" style={{ borderColor: 'var(--rz-border-light)' }}>
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-[13px] font-bold" style={{ color: 'var(--rz-text-primary)' }}>Help Center</h3>
                  <p className="text-[10px]" style={{ color: 'var(--rz-text-muted)' }}>Everything you need to get started</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex border-b px-5 flex-shrink-0" style={{ borderColor: 'var(--rz-border-light)' }}>
              {TABS.map(tab => (
                <button
                  ...
                  className={`px-3 py-2.5 text-[11px] font-bold transition-colors duration-150 border-b-2 -mb-px ${
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
              {activeTab === 'quickstart' && (
                <div className="space-y-3">
                  <p className="text-[11px] mb-3" style={{ color: 'var(--rz-text-muted)' }}>Follow these steps to reconcile your first batch:</p>
                  {QUICK_STEPS.map((step, i) => (
                    <div key={step.num} className="flex items-start gap-3 stagger-item" style={{ animationDelay: `${i * 0.06}s` }}>
                      <div className="w-7 h-7 rounded-md flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0" style={{ background: 'var(--rz-blue)' }}>
                        {step.num}
                      </div>
                      <div className="pt-0.5">
                        <h4 className="text-[12px] font-bold" style={{ color: 'var(--rz-text-primary)' }}>{step.label}</h4>
                        <p className="text-[10px] mt-0.5" style={{ color: 'var(--rz-text-muted)' }}>{step.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {activeTab === 'shortcuts' && (
                <div className="space-y-2">
                  <p className="text-[11px] mb-3" style={{ color: 'var(--rz-text-muted)' }}>Keyboard shortcuts:</p>
                  {SHORTCUTS.map((sc, i) => (
                    <div key={sc.label} className="flex items-center justify-between py-2 px-3 rounded-lg border stagger-item" style={{ borderColor: 'var(--rz-border)', animationDelay: `${i * 0.05}s` }}>
                      <span className="text-[12px] font-semibold" style={{ color: 'var(--rz-text-secondary)' }}>{sc.label}</span>
                      <div className="flex items-center gap-1">
                        {sc.keys.map(k => (
                          <kbd key={k} className="px-1.5 py-0.5 bg-gray-50 border border-gray-200 rounded text-[10px] font-bold min-w-[22px] text-center" style={{ color: 'var(--rz-text-muted)' }}>
                            {k}
                          </kbd>
                        ))}
                      </div>
              ...
              {activeTab === 'features' && (
                <div className="space-y-1.5">
                  <p className="text-[11px] mb-3" style={{ color: 'var(--rz-text-muted)' }}>What each section does:</p>
                  {FEATURES.map((f, i) => {
                    const Icon = f.icon
                    return (
                      <div key={f.label} className="flex items-center gap-2.5 py-2 px-3 rounded-lg border hover:bg-gray-50 transition-colors stagger-item" style={{ borderColor: 'var(--rz-border)', animationDelay: `${i * 0.04}s` }}>
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="text-[11px] font-bold" style={{ color: 'var(--rz-text-primary)' }}>{f.label}</h4>
            ...
            <div className="px-5 py-3 border-t flex-shrink-0" style={{ borderColor: 'var(--rz-border-light)' }}>
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-3 py-2 border rounded-md text-[12px] font-semibold hover:bg-gray-50 transition-colors"
