    ...
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Persistent audit history of all AI investigations, recommendations, and human approvals.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleClearLogs} className="px-3 py-2 bg-red-50 hover:bg-red-100 text-[#EF4444] border border-red-200 rounded-lg text-xs font-extrabold flex items-center gap-1.5 transition-colors">
        ...
        <div className="overflow-x-auto">
          {logs.length === 0 ? (
            <div className="p-12 text-center text-gray-500 text-sm">
              ...
              <p className="text-xs text-gray-400 mt-1">Audit logs will automatically appear when AI investigations or human approvals execute.</p>
            </div>
          ) : (
              ...
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id} className="border-b border-[#E8EEF5] hover:bg-[#E6F0FF]/30 transition-colors">
                    <td className="px-6 py-4 text-xs text-gray-500 font-medium whitespace-nowrap">{log.timestamp}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded text-[11px] font-extrabold uppercase tracking-wider ${
                    ...
                    <td className="px-6 py-4 font-mono text-xs font-bold text-[#0065FF]">{log.investigation_ref || log.entity_id}</td>
                    ...
                    <td className="px-6 py-4 text-xs text-gray-600 max-w-sm">{log.reason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
